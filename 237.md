...
```txt
However, if there are multiple perps and each perp starts from PnL >= 0 and the account is safe, at least one of the perp has PnL < 0 when the account becomes unsafe.
```
A good comprehensive check might have to include primary credit, secondary credit, and netPositionValuePrime.
